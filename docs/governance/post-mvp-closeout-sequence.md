...
它的目标不是把所有文档一次性补平，而是把项目收口到一个**可决策状态**。

也就是说，这一步只需要做到：

> **不重新翻聊天记录，也能说清“现在站在哪儿”。**

### B. 路线绑定的 current truth 收口

这一步是在**下一阶段主线已经选定之后**才做。

它回答的是：

- 为了让这条主线顺利推进，哪些文档必须先补准
- 哪些边界如果不先锁清，后续开发会被误导

也就是说：

> **先收尾到可决策，再选路线；路线一旦确定，再围绕这条路线定向补齐 current truth。**
...
> **先做“当前阶段收尾总结” → 再做“下一阶段选线” → 再做“路线绑定的 current truth 收口”。**

一句话讲清：

> **不是先靠路线决定怎么收尾，而是先把当前收尾到足以做路线决策；然后再由路线决定下一轮要补准哪些 truth。**

## 1.2 阶段切换决策图

如果要把上面的逻辑再压缩成一张判断图，可以直接用下面这张：
...
baseline / backlog     +-----+-----+
    ...
    +----------------> 否          是
                  ...
                  继续压缩问题、   围绕这条主线，
                  比较候选主线     定向补齐
                  直到只剩一条     current truth
                       |           |
                       +-----+-----+
                             |
                             v
                    形成 next-phase kickoff
                    + PRD + test spec + breakdown
                             |
                             v
                           开始执行
```
...
> **先收尾到“能做决定”，再选线到“只剩一条主线”，再补文档到“这条主线不再被误导”。**
