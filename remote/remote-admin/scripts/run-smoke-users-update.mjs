...
const artifactDir = resolve(
  cliArtifactDir ||
    envArtifactDir ||
    join(repoRoot, 'discss', 'artifacts', `remote-users-update-smoke-${formatTimestamp()}`)
);

await mkdir(artifactDir, { recursive: true });

console.log(`[INFO] Remote users update smoke artifact dir: ${artifactDir}`);
...
const child = spawn(process.execPath, [join(__dirname, 'smoke-users-update.mjs'), artifactDir], {
  cwd: repoRoot,
  stdio: 'inherit',
});
...
console.log(`[INFO] Remote users update smoke passed. Evidence saved to: ${artifactDir}`);
