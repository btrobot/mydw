...
const artifactDir = resolve(
  cliArtifactDir ||
    envArtifactDir ||
    join(repoRoot, 'discss', 'artifacts', `remote-users-update-multi-smoke-${formatTimestamp()}`)
);

await mkdir(artifactDir, { recursive: true });

console.log(`[INFO] Remote multi-user update smoke artifact dir: ${artifactDir}`);
...
const child = spawn(process.execPath, [join(__dirname, 'smoke-users-update-multi.mjs'), artifactDir], {
  cwd: repoRoot,
  stdio: 'inherit',
});
...
console.log(`[INFO] Remote multi-user update smoke passed. Evidence saved to: ${artifactDir}`);
