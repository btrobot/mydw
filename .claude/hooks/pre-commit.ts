// Pre-commit validation hook for dewugojin
...
for (const file of files) {
  ...
  if (file.match(/\.(ts|tsx)$/)) {
    ...
    if (anyUsages.length > 0) {
      warn(`TypeScript uses 'any' type: ${file}`);
      anyUsages.slice(0, 3).forEach((line) => {
        warn(`  Line ${line}: ${lines[line - 1].trim()}`);
      });
      warningCount++;
    }
  }
  ...
  if (file.match(/\.py$/)) {
    ...
    if (printUsages.length > 0) {
      ...
      warn("  Please use loguru.logger instead");
      warningCount++;
    }
  }
}
