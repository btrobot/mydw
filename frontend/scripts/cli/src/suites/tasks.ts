 ...
 * 获取任务测试用例
 */
export function getTaskTests(): TestCase[] {
  return [
    {
      name: 'GET /api/tasks/ (任务列表)',
      fn: () => GET('/api/tasks'),
    },
    {
      name: 'GET /api/tasks/stats (统计)',
      fn: () => GET('/api/tasks/stats'),
    },
  ]
}
 ...
 * 运行任务测试
 */
export async function runTaskTests(): Promise<TestResult> {
  const tests = getTaskTests()
  let passed = 0
  let failed = 0
