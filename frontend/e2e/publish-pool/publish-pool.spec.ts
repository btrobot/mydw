...
test.describe('Phase C publish pool visibility', () => {
  test('workbench can filter pool candidates and detail keeps pool history in advanced diagnostics', async ({ page }) => {
    ...
    await expect(page.getByTestId('creative-current-publish-package-summary')).toContainText('Package #302')
    await expect(page.getByTestId('creative-current-publish-package-summary')).toContainText('冻结商品名：Classic Hoodie 春季轻运动版')
    ...
    await expect(page.getByTestId('creative-pool-item-freeze-501')).toContainText('冻结文案：春季轻装，轻盈上身，即刻出发。')
  })
})
